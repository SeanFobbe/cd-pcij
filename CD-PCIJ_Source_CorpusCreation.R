...
#+
#'## Summary
#' This R script downloads and processes the full set of decisions and appended opinions rendered by the Permanent Court of International Justice (PCIJ) --- as published in Series A, B and AB on the website of the International Court of Justice (ICJ) --- into a rich and structured human- and machine-readable data set. It is the basis for the \textbf{\datatitle\ (\datashort )}.
#'
#' All data sets created with this script will always be hosted permanently open access and freely available at Zenodo, the scientific repository of CERN. Each version is uniquely identified with a persistent Digitial Object Identifier (DOI), the \emph{Version DOI}. The newest version of the data set will always available via the link of the \emph{Concept DOI}: \dataconcepturldoi
...
#' 
#' Please refer to the Codebook regarding the relative merits of each variant. Unless you have very specific needs you should only use the variants denoted \enquote{TESSERACT} or \enquote{ENHANCED} for serious work.
#' 
...
#'## System Requirements
#'
#' You must have **R** and all **R packages** listed under the heading \enquote{Load Packages} installed.
#'
